 ...
 * DBConnectionBroker
 *
 * Copyright (C) 1999 Marc A. Mnich <marc@javaexchange.com>
 * 
...
public class DbConnectionBroker implements Runnable
{
		...
		throws IOException
	{
		...
		catch (IOException e1)
		{
			try
			{
				log =
					new PrintWriter(
						new FileOutputStream("DCB_" + System.currentTimeMillis() + ".log"),
						true);
		...
		BufferedWriter pidout =
			new BufferedWriter(new FileWriter(logFileString + "pid"));
		pidout.write(pid);
		pidout.close();
	...
	private void createConn(int i) throws SQLException
	{
		...
		{
			System.out.println(" failed in: DbConnectionBroker " + e2.getMessage());
			e2.printStackTrace();
		}
	...
	public void destroy(int millis) throws SQLException
	{
		...
		if (useCount > 0)
		{
			//bt-test successful
			String msg =
				"Unsafe shutdown: Had to close "
					+ useCount
					+ " active DB connections after "
					+ millis
					+ "ms";
			log.println(msg);
		...
		if (available)
		{
			...
			for (int outerloop = 1; outerloop <= 10; outerloop++)
			{
				...
				else
				{
					...
					log.println(
						"-----> Connections Exhausted!  Will wait and try "
							+ "again in loop "
