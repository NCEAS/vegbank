...
public class nameQuery
{
	...
	public void getName(String input_s)
	{
		...
		try
		{
			results =
				query.executeQuery(
					"select name_id, name, symbol, commonname, family, AUTHORNAME  from name where name like '%"
						+ input_s
						+ "%'");
			int i = 0;
			while (results.next())
		...
		{
			System.out.println("Query failed nameQuery.getName " + e.getMessage());
		}
	...
	public void getNameId(int input_s)
	{
		...
		try
		{
			results =
				query.executeQuery(
					"select name_id, name, symbol, commonname, family, AUTHORNAME  from name where name_id  = "
						+ input_s);
			int i = 0;
			while (results.next())
			{
