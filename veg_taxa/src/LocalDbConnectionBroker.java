	...
	public static Connection manageLocalDbConnectionBroker(String action)
	{
		...
		if (action.equals("initiate"))
		{
			try
			{
				...
				//get a connection that will be closed
			...
			{
				System.out.println(
					" failed in: manageLocalDbConnectionBroker " + e.getMessage());
				e.printStackTrace();
			}
		}

		else
			if (action.equals("getConn"))
			{
				...
				{
					System.out.println(
						"LocalDbConnectionBroker: grabbing a new connection from pool");
					...
					if (myBroker.getSize() == g.maxConnections)
					{
						...
						{
							System.out.println(
								"LocalDbConnectionBroker: re-starting the connection pooling");
							myBroker =
								new DbConnectionBroker(
									g.driverClass,
									g.connectionString,
				...
				if (action.equals("releaseConn"))
				{
					...
					{
						System.out.println(
							"failed in: manageLocalDbConnectionBroker realease "
								+ e.getMessage());
						e.printStackTrace();
					}
				}
					...
					if (action.equals("destroy"))
					{
						...
						{
							System.out.println(
								"failed in: manageLocalDbConnectionBroker destroy"
									+ e.getMessage());
							e.printStackTrace();
						}
					...
					{
						System.out.println(
							"LocalDbConnectionBroker.manageLocalDbConnectionBroker: unrecognized action"
								+ action);
					}
