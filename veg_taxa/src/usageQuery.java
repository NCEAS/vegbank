...
public class usageQuery
{
	...
	public void getNameId(int input_s)
	{
		...
		{
			System.out.println(
				"failed in the  dbConnect call" + e.getMessage());
		}
		...
		try
		{
			results =
				query.executeQuery(
					"select NAMEUSAGEID, circum_id, name_id, party_id, startDate, stopdate, acceptence  from name_usage where name_id = "
						+ input_s);
			int i = 0;
			while (results.next())
			{
		...
		{
			System.out.println(
				"Query failed usageQuery.getNameId " + e.getMessage());
		}
	...
	public void getCircumId(int input_s)
	{
		...
		try
		{
			results =
				query.executeQuery(
					"select  NAMEUSAGEID, circum_id, name_id, party_id, startDate, stopdate, acceptence  from name_usage where circum_id = "
						+ input_s);
			int i = 0;
			while (results.next())
			{
		...
		{
			System.out.println(
				"Query failed usageQuery.getCircumId " + e.getMessage());
		}
