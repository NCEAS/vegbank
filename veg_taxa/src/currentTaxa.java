...
m.getName(firstName);
	...
	/*get the circum_id from the  uses of that name*/ 
		...
		if (usagePartyId[i3]==input_party) {	
				...
				System.out.println("  nameUsageId: "+nameUsageId[i3]+" circumId: "+circumId[i3]+" usagePartyId: "+usagePartyId[i3]+" acceptence: "+nameAcceptence[i3]);
			...
			String status[]=m2.outStatus;
				...
				System.out.println("    statusCircumId: "+statusCircumId[i4]+" staus: "+status[i4]+" partyID: "+statusPartyId[i4]+" "+statusStartDate[i4].substring(0, 4)+"-"+statusStopDate[i4].substring(0, 4));
				
				
				/*check to see if usage is non standard*/
				if (nameAcceptence[i3].trim().equals("A"))  {  //if 3
					...
					String acceptedStopDate[]=m3.outStopDate;
						...
						for (int i5=0; i5<acceptedUsageIdNum; i5++) {  //for 4
							
							if (acceptedAcceptence[i5].trim().equals("S")  && (acceptedPartyId[i5]==input_party))  {  //if 4 
							System.out.println("NameId: "+acceptedNameId[i5]+" party id: "+acceptedPartyId[i5]+" acceptence: "+acceptedAcceptence[i5]+" acceptedCircumId:"+acceptedCircumId[i5]+" startDate: "+acceptedStartDate[i5]+" acceptedStopDate: "+acceptedStopDate[i5]);
							...
							for (int i6=0; i6<acceptedNameNum; i6++) {  //for 5
								System.out.println("Accepted taxonName: "+acceptedNameArray[i6]);
								/*write the output to the public variable*/
								outNonStandardUsage[outNonStandardUsageNum]=(nameArray[ii]+"|"+acceptedNameArray[i6]+"|"+acceptedStartDate[i5].substring(0, 4)+"|"+acceptedStopDate[i5].substring(0, 4));
				...
				/*else the usage is standard and the input string is equal to the current taxon*/
					...
					outStandardUsage[outStandardUsageNum]=(nameArray[ii]+"|"+nameArray[ii]+"|"+statusStartDate[i4].substring(0, 4)+"|"+statusStopDate[i4].substring(0, 4));
					outStandardUsageNum++;
					}
				} //end else
               ...
               usageQuery m1 = new usageQuery();
                ...
                if (usagePartyId[i3]==input_party) {
                        ...
                        String status[]=m2.outStatus;
                                ...
                                if (status[i4].equals("S")) {outStandardCorrelation[outStandardCorrelationNum]=(nameAcceptence[ii]+"|"+nameArray[ii]+"|"+statusCircumId[i4]+"|"+status[i4]+"|"+statusPartyId[i4]+"|"+statusStartDate[i4].substring(0, 4)+"|"+statusStopDate[i4].substring(0, 4)); outStandardCorrelationNum++;}
                              ...
                              System.out.println("       correlationId: "+correlationId[i5]+" congruence: "+congruence[i5]+" statusCircum2: "+statusCircum2[i5]);
                                        ...
                                        int correlationCircumNum=m4.outStatusNum;
                                                for (int i6=0; i6<correlationCircumNum; i6++) {
                                                        ...
                                                        System.out.println("        correlationCircumId: "+correlationCircumId[i6]+" correlationCircumStatus: "+correlationStatus[i6]);
							...
							String correlationAcceptence[]=m5.outAcceptence;
                                                        ...
                                                        System.out.println("           NameId associated with correlationCircum: "+correlationNameId[i7]+" usageParty: "+correlationPartyId[i7]+" startDate: "+correlationStartDate[i7].substring(0, 4)+"stoprDate: "+correlationStopDate[i7].substring(0, 4));
                                                                ...
                                                                System.out.println("              Name associated: "+currentName[i8]);
                                                                System.out.println("            ");
                                                                System.out.println("            ");
...
outNonStandardCorrelation[outNonStandardCorrelationNum]=(nameAcceptence[i3]+"|"+nameArray[ii]+"|"+statusCircumId[i4]+"|"+correlationId[i5]+"|"+congruence[i5]+"|"+statusCircum2[i5]+"|"+correlationCircumId[i6]+"|"+correlationStatus[i6]+"|"+correlationNameId[i7]+"|"+correlationPartyId[i7]+"|"+correlationStartDate[i7].substring(0, 4)+"|"+correlationStopDate[i7].substring(0, 4)+"|"+currentName[i8]+"|"+correlationAcceptence[i7]); outNonStandardCorrelationNum++;
	}
