...
<xsl:comment>Starting view <xsl:value-of select="$view" /> with number:<xsl:value-of select="$viewNum" /></xsl:comment>
<xsl:if test="string-length($view)&gt;0">
<xsl:value-of select="$view"/>IS VIEW:
<xsl:call-template name="doEnts" >
  <xsl:with-param name="view" select="$view" /></xsl:call-template>
<!-- call next view -->
...
<xsl:if test="count(attribute[translate(attForms/formShow/@name,$alphahigh,$alphalow)=$view])&gt;0">
...
<redirect:write file="{$pathToWrite}{$currEnt}_{$view}_head.jsp"> 
                  ...
                  <xsl:for-each select="attribute[translate(attForms/formShow/@name,$alphahigh,$alphalow)=$view  and attForms/formShow&gt;0]">
...
<xsl:comment>END WRITE FILE: <xsl:value-of select="$currEnt" />_<xsl:value-of select="$view" />_data.jsp</xsl:comment>             
  ...
  <xsl:template match="/dataModel">
    <html>
   <xsl:call-template name="doViews" ><xsl:with-param name="viewNum">1</xsl:with-param></xsl:call-template>
   </html>
  </xsl:template>
