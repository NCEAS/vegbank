...
public class project_Action extends org.apache.struts.action.Action
{
		...
		HttpServletRequest request, HttpServletResponse response) throws Exception
	{
		...
		try
		{
			...
			{
				manager.deleteProject(((SuperForm)form).getFieldValue(((SuperForm)form).getPrimaryKey()));
				((SuperForm)form).clearForm();
				proposal_Form f=(proposal_Form)session.getAttribute("proposal_form");
				if(f!=null) f.setPROJECT_ID(BeanManager.UNKNOWN_ID);
			}
			else
