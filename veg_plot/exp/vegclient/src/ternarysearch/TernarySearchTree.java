...
public class TernarySearchTree {
	...
	*   Only keys for nodes having non-null data 
	...
	*/
	...
	/**
	 * method that basically returns the same as the other methods with the same 
	 * name except that this returns a vector containing the elements
	 *
	 * @param prefix -- the prefix that should be matched
	 * @param  elementNumber -- the number of elements that should be returned
	 * @return  a vector with the matching elements limited to the size of the
	 * vector
	 */
	 public Vector matchPrefixStringVec(String prefix, int elementNumber) 
	 {
		String s = matchPrefixString(prefix, defaultNumReturnValues);
		StringTokenizer t = new StringTokenizer(s, "\n" );
    StringBuffer  sb  = new StringBuffer();
		Vector vec = new Vector();
		String buf = null;
		int count = 0;
    while ( t.hasMoreTokens() && count < elementNumber )
    {
			buf = t.nextToken();
			vec.addElement( buf );
			count++;
	...
	*/
	public String matchPrefixString(String prefix, int numReturnValues) {
		System.out.println("number if return vals: "+  numReturnValues);
		TSTNode startNode = getNode(prefix);
