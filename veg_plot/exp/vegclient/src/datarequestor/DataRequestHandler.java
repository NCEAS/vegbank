...
//	System.out.println("*compond query: "+compoundQuery);
 ...
 readResults("summary.xml");
	...
	/**
	 * method that takes a vector containing a series of hashtables, which ecahc
	 * contain 3 strings: criteria, opererator and a value, which are printed to
	 * an xml document that represents an extended query and then is issued
	 * against the database from here.  The results are returned to an xml file
	 * that is also spesified in the query xml document
	 * @param 
	 */	 
	public int handleExtendedQuery(Vector vec) 
	{
		int results = 0;
		...
		try
		{
			...
			String clientType = "clientApplication";
			String requestDataFormatType  = "xml";
			//the 3 vectors corresponding to the criteria, operator, and values
			Vector criteriaVec = new Vector();
...
//				System.out.println("ELEMENTS TO BE PROCESSED: "+enum.toString() );
				for (int i=0; i<vec.size(); i++) 
				{
					...
					// and 'value'
					Hashtable triple = (Hashtable)vec.elementAt(i);
  				System.out.println("Triple : "+ triple );
					if ( triple.containsKey("criteria")  
						|| triple.containsKey("operator") 
						|| triple.containsKey("value"))
					{
			...
			//			}
						...
						criteriaVec.addElement( triple.get("criteria").toString() );
						operatorVec.addElement( triple.get("operator").toString() );
						valueVec.addElement( triple.get("value").toString() );
				...
				//add the vectors to the hashatable
				extendedParamsHash.put("criteria", criteriaVec );
				extendedParamsHash.put("operator", operatorVec );
				extendedParamsHash.put("value", valueVec );
						
				//compose the query
				composeExtendedQuery( extendedParamsHash ) ;
				//issue the extended query
				results = issueQuery("extendedQuery");
				...
				{ 
					System.out.println("<br> <b> Please try another query </b> <br>"); 
					System.out.println("<a href = \"/examples/servlet/pageDirector?pageType=DataRequestServlet\">"
	...
	private String composeExtendedQuery( Hashtable extendedParamsHash ) 
	{
		try 
		{
			...
			else 
			{
				...
				System.out.println("cannot compose extended query: " + extendedParamsHash.toString() );
				
			}
			sb.append("</extendedQuery>"+"\n");
			sb.append("<requestDataType>summary</requestDataType> \n");
			sb.append("<resultType>summary</resultType> \n");
			sb.append("<outFile>summary.xml</outFile> \n");
			sb.append("</dbQuery>"+"\n");
...
/**
 *  Method to use dbAccess to issue the query to the database from the xml file
 *  created in the DataRequestHandler 
 *
 * @param queryType - the type of query to be sent through the dataAccess module
 * including simpleQuery (one attribute) and compoundQuery (multiple attributes) 
 */
	private int issueQuery (String queryType) 
	{
		int results =0;
...
//			dba.accessDatabase("/jakarta-tomcat/webapps/examples/WEB-INF/lib/query.xml", "/jakarta-tomcat/webapps/examples/WEB-INF/lib/querySpec.xsl", "query");
		...
		{
			//call the plot access module
			dba.accessDatabase("query.xml", "querySpec.xsl", "compoundQuery");	
			//grab the result set from the dbAccess class
			queryOutput=dba.queryOutput;
			queryOutputNum=dba.queryOutputNum;
		}
		...
		{
			//call the plot access module
			dba.accessDatabase("query.xml", "querySpec.xsl", "extendedQuery");	
			//grab the result set from the dbAccess class
			queryOutput=dba.queryOutput;
			queryOutputNum=dba.queryOutputNum;
		}
		...
		{ 
			//call the plot access module
			dba.accessDatabase("commQuery.xml", "querySpec.xsl", "simpleCommunityQuery");	
			//grab the result set from the dbAccess class
			queryOutput=dba.queryOutput;
			queryOutputNum=dba.queryOutputNum;
		}
		...
		{ 
			//call the plot access module
			dba.accessDatabase("taxonQuery.xml", "querySpec.xsl", "simplePlantTaxonomyQuery");	
			//grab the result set from the dbAccess class
			queryOutput=dba.queryOutput;
			queryOutputNum=dba.queryOutputNum;
		}
		...
		{
			System.out.println("DataRequestHandler could not handle query type: "
				+queryType);
		}
		return( queryOutputNum );
	}
