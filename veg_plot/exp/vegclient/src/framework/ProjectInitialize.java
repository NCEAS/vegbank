    ...
    public ProjectInitialize() 
		{
				...
				setTitle(" Data-Loading Workflow Manager -- build: @release@");
        initComponents ();
        pack ();
...
//			projectNamePanel.setPreferredSize(new Dimension(250, 80));
			...
			//the text area with the project description	
      ...
      getContentPane().add(mainPane);
			...
			//try to get a project file
			projectFileSelector.addActionListener(new java.awt.event.ActionListener() 
			{
        public void actionPerformed(ActionEvent evt) 
				{
					//get the file
					fileChooser(evt, projectFileValue);
        	//handleManagerAcivator(evt);
					
					//make sure that the file is a valid VegProject xml document
					if ( projectFileValue.getText().indexOf("xml") < 3)
					{
						pm.debug(0, "not an xml file!");
					}
					else
					{
						try
						{
							//create a new instance of the VegProject
							VegProject project = new VegProject( projectFileValue.getText() );
							//update the projectDescription
							if (project.getProjectName() != null && 
								project.getProjectDescription() != null )
							{
								projectNameValue.setText(project.getProjectName() );
								projectDescriptionValue.setText(project.getProjectDescription() );
							}
							else
							{
								pm.debug(0, "not a valid Project file");
							}
						}
						catch (Exception e) 
						{
	...
	/**
		* method to handle the action related to the activation of the 'start 
		* manager' button
		* 
		* @param evt -- the actionEvent responsible for triggering this method
		*
		*/
		private String handleManagerAcivator(ActionEvent evt)
		{
			...
			try
			{
				System.out.println("Start Manager Button Activated");
