 ...
 */
	public static void main(String[] args) 
	{
		...
		if (args.length < 1) 
		{
			System.out.println("Usage: java AsciiDataLoader  [ascifile] [headerfile] ");
			System.exit(0);
		}
		else 
		{
	...
	public void loadAsciiData(String dataFile, String headerFile) 
	{
		...
		for (int row=0;row<dataVector.size(); row++) 
		{
			for (int i=0;i<attributeVector.size(); i++) 
			{
				...
				inputValue[i] = getDataValue(dataVector, row, i, delimeterString);
			}
			...
			System.out.println("Insert: "+ insertString + " " + attributeString);	
		}
	} 
	catch 	( Exception e )
	{
		System.out.println("failed at: AsciiDataLoader.loadAsciiData  "
		+e.getMessage());
		e.printStackTrace();
	}
}
 ...
 */
	public void getSchema(Vector schemaVector) 
	{
		...
		{		
			...
			{
				StringTokenizer t = new StringTokenizer(schemaVector.elementAt(i).toString(), " ");
 ...
 * @param rowNumber -- the 
 */
	public String getDataValue(Vector dataVector, int rowNumber, int columnNumber,
...
//		System.out.println("searching form column number: "+columnNumber);
		...
		StringTokenizer t = new StringTokenizer(dataVector.elementAt(rowNumber).toString(), delimeterString);
		String buf=null;
