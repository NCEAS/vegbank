...
/*  
  public int readLine2(byte b[], int off, int len)
      throws IOException
  {
    int remain = len;
    int avail = count - pos;
    ...
    // no need to pass instance vars, and method can increment pos for us
    // or is a private static method that much faster than a private?
    copy = copyLine(buf, pos, b, off, copy);
    if (copy == 0)
      return -1;
    pos += copy;
    remain -= copy;
    ...
    int total = copy;
    copy = 0;
    // no point to maintain total and remain is there?
    for(; remain > 0 && b[(off + total) - 1] != 10; total += copy)
    {
      fill();
      avail = count - pos;
      if(avail <= 0)
      ...
      copy = Math.min(avail, remain);
      copy = copyLine(buf, pos, b, off + total, copy);
      pos += copy;
      remain -= copy;
    }
  ...
  private static int copyLine(byte src[], int src_pos, byte dest[], int dest_pos, int dest_len)
  {
    int pos;
    // two incs and a cmp per loop; hmmmm, surely we only need one inc?
    for(pos = src_pos; dest_len-- > 0 && src[pos++] != 10;)
      ;
    
    System.arraycopy(src, src_pos, dest, dest_pos, pos - src_pos);
    return pos - src_pos;
  }
*/  
