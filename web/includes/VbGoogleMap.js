  ...
  function VbGMapCSV(csvstring,mapDivId,minLat,maxLat,minLng,maxLng, blnSetBoundsBasedOnPlots, blnTryToMarkProjects, zoomMapId, blnAutoDrawAcc, blnAutoDrawBounds) {
      ...
      //  var xml = GXml.parse(data);
        ...
        for (var i = 0; i < markers.length; i++) {
           ...
           var thisPlotCSV = CSVToArray(markers[i]);
...
// modified from http://www.bennadel.com/blog/1504-Ask-Ben-Parsing-CSV-Strings-With-Javascript-Exec-Regular-Expression-Command.htm
...
function CSVToArray( strData, strDelimiter ){
	...
	// while (arrMatches = objPattern.exec( strData )){ // this fails with empty length strings!
     while ((arrMatches = objPattern.exec( strData ))  && (strData.length != 0) ){
